...
- Per-file error codes (exact strings): `unsupported_type`, `too_large`, `empty_file`, `corrupt`, `timeout`, `engine_error`, `engine_unconfigured`. Request-level 400 only for: not multipart, zero files, > MaxFilesPerRequest, malformed hints JSON, hints part over its read cap, declared request body over MaxRequestBytes.
